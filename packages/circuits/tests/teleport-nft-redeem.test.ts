...
describe('Teleport NFT Redeem Circuit', () => {
  ...
  beforeAll(async () => {
    circuit = await wasm_tester(path.join(__dirname, '../examples/teleport-nft-redeem.circom'), {
      recompile: true,
    ...
    payload = {
      ...
      nbf: 1733218047,
      nonce: 'asdfghjkl',
      picture: 'https://lh3.googleusercontent.com/a/ACg8ocLEfBS88uysDlR62qW_ysr-eH2UjKjX5RJ-NCq5CyUBIu3xOw=s96-c',
  ...
  it('should verify a teleport-nft-redeem JWT', async () => {
    ...
    const verifierInputs = await generateJWTAuthenticatorInputs(rawJWT, publicKey, BigInt(1024), {
      maxMessageLength: 1024,
      exposeGoogleId: true,
    });

    const { accountCode, codeIndex, iatKeyStartIndex, ...filteredCircuitInputs } = verifierInputs;
